...
documents/
  index.html           Browsable library of all 637 documents
...
|---|---|---|
| `articles.json` | `articles` | 92 (five more every day — see `docs/articles.md`) |
| `checklists.json` | `checklists` | 14 (two more every day — see `docs/checklists.md`) |
| `challenges.json` | `challenge_templates` + `challenge_days` | 22 themes (365 daily to-dos each, 8,030 in all; two more themes every day — see `docs/challenges.md`) |
...
| `therapist-resources.json` | `therapist_resources` (the clinician library on the therapist dashboard; not rendered into `documents/`) | 500 (five more of every kind and five on each of eighteen topics every day — see `docs/therapist-resources.md`) |

The 372 journal prompts are grouped into 11 themed collections at build time
(keyword rules live in `THEMES` in the generator), which is why 637 documents come
out of 998 source rows (the journal prompt rows collapse into 11 collections).
