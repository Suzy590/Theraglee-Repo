...
| Therapists see only gender, age, topics, session preference and zip code, and only with opt-in; the member's real name reaches a therapist only in a reply the member sends | `site/assets/match.js`, the `member_discovery` view (no name column) and `outreach_replies.member_name` |
